...
ESX.RegisterUsableItem('boosterbox', function(source)
	...
	xPlayer.removeInventoryItem('boosterbox', 1)
        	TriggerClientEvent("Cards:Client:OpenCards", source, 'boosterbox')
...
AddEventHandler('Cards:Server:rewarditem', function()
   ...
   -- print(randomChance) -- if you wanna see the random chance
	...
	if card ~= '' then
		...
		ESX.Trace('There is a problem in cards with ' .. xPlayer .. " !")
	end
...
AddEventHandler('Cards:Server:GetPokemon', function(pokemon)
	local xPlayer = ESX.GetPlayerFromId(source)
	local pokemonNameItem = xPlayer.getInventoryItem(pokemon)
	local pokemonName = pokemonNameItem.label

	if pokemon ~= nil then
-- 		TriggerClientEvent("inventory:client:ItemBox", pokemon, "add") ???????????
...
AddEventHandler("Cards:server:badges", function(type)
	...
	for k, v in pairs(Config.Badge[type].cards) do
		if xPlayer.hasItem(k) then
			if xPlayer.hasItem(k).count < v then
				canBadge = false
				xPlayer.showNotification('Come back when you have all the items for the ' .. Config.Badge[type].label .. ' !')
			end
		else 
			canBadge = false
			xPlayer.showNotification('Come back when you have all the items for the ' .. Config.Badges[type].label .. ' !')
			break
		end
	end
...
ESX.RegisterUsableItem('pokebox', function(source, item)
	local xPlayer = ESX.GetPlayerFromId(source)
	TriggerClientEvent("Cards:client:UseBox", source)
	ESX.Trace('A player opened a pokebox')
end)
...
AddEventHandler("Cards:sellItem", function(itemName, amount, price)
	...
	ESX.Trace("Someone just sold " .. amount .. " " .. itemName .. " card Item for $" .. price)
